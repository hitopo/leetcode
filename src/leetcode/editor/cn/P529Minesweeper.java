    ...
    class Solution {
        ...
        public char[][] updateBoard(char[][] board, int[] click) {
            ...
            if (board[clickX][clickY] == 'M') {
                ...
                clickPosition(board, clickX, clickY);
            }
            return board;
        }

        private void clickPosition(char[][] board, int clickX, int clickY) {
            ...
            int bombCnt = 0;
            for (int i = 0; i < 8; i++) {
                int newX = clickX + dx[i];
                int newY = clickY + dy[i];
                // 统计数量
                if (locInBoard(board, newX, newY) && board[newX][newY] == 'M') {
                    bombCnt++;
                }
            }
            if (bombCnt != 0) {
                board[clickX][clickY] = (char) ('0' + bombCnt);
            } else {
                board[clickX][clickY] = 'B';
                for (int i = 0; i < 8; i++) {
                    int newX = clickX + dx[i];
                    int newY = clickY + dy[i];
                    if (locInBoard(board, newX, newY)) {
                        clickPosition(board, newX, newY);
                    }
                }
            }
        }

        private boolean locInBoard(char[][] board, int x, int y) {
